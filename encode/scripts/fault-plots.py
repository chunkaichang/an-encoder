...
def plot_test(name, data, outdir, ax, legend=False):
    ...
    ax.set_ylim([0,1])
    ax.set_xticks(ticks)
    ax.set_xticklabels(profs*2, rotation=90, size=8)

    if legend:
      ax.legend(loc="lower right", ncol=1, fontsize=5)

    return handles

def plot_results(results, outdir, legend):
    fig, axes = plt.subplots(1, len(results.keys()), sharey=True, figsize=(9,4))
    fig.set_facecolor("w")
    index = 0
    for name in tests:
      data = results[name]
      handles = plot_test(name, data, outdir, axes[index], index == 3)
      index += 1
...
if __name__ == "__main__":
    ...
    dirnames = dict()
    dirnames["REG"] = args.REG
    dirnames["VAL"] = args.VAL
    dirnames["ADDR"] = args.ADDR
    dirnames["TXT"] = args.TXT
    dirnames["CF"] = args.CF
    ...
    if args.unpickle is not "":
        ...
        res = collect_profiles("encoded", profiles, dirnames, False)
