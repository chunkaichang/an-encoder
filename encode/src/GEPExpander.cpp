...
namespace {
    ...
    public:
        ...
        ExpandGetElementPtr(Coder *c) : BasicBlockPass(ID), C(c) {}
...
static void ExpandGEP(GetElementPtrInst *GEP, DataLayout *DL, Type *PtrType, Coder *C) {
    ...
    Instruction *Ptr = new PtrToIntInst(GEP->getPointerOperand(), PtrType,
		...
		if (dyn_cast<GlobalValue>(GEP->getPointerOperand()->stripPointerCasts()))
			Ptr = dyn_cast<Instruction>(C->createEncode(Ptr, GEP, true));
...
bool ExpandGetElementPtr::runOnBasicBlock(BasicBlock &BB) {
    ...
    for (BasicBlock::InstListType::iterator Iter = BB.begin();
        ...
        if (GetElementPtrInst *GEP = dyn_cast<GetElementPtrInst>(Inst)) {
            Modified = true;
            ExpandGEP(GEP, &DL, PtrType, C);
        }
    }
    return Modified;
}
BasicBlockPass *createExpandGetElementPtrPass(Coder *c) {
    return new ExpandGetElementPtr(c);
}
